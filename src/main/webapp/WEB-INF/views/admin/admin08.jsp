  ...
  AUIGrid.bind(salesGrid,"cellEditEnd", function(event) {
    ...
    $.ajax({
          ...
          success : function(data) {
            getSalesInfo();
            getYearInfo();
            AUIGrid.setCellValue(salesGrid,event.rowIndex, "outstanding", event.item.outstanding - event.item.income);
            AUIGrid.setCellValue(salesGrid,event.rowIndex, "deposit", event.item.deposit + event.item.income);
            AUIGrid.setCellValue(salesGrid,event.rowIndex, "income","");
            AUIGrid.updateGrouping(salesGrid);
...
var footerLayout = [ {
    ...
    dataField : "totalOutstanding",
    positionField : "totalOutstanding",
    operation : "SUM",
    formatString : "#,##0"
  }
];
...
function getSalesInfo(){
  ...
  $.ajax({
        ...
        success : function(data) {
          ...
          $("#outstandingSum").text(numberComma(data.outstandingsum) + " 원");
          if(data.sales != 0)
            $("#outstanding").text(numberComma(data.total - data.deposit) + "원("+ ((data.sales - data.deposit) / data.sales * 100).toFixed(2) + " %)");
          else 
          $("#outstanding").text( "0 %");
      ...
      <section class="section02">
        ...
        <div class="filter_area admin_filter">
          ...
          <div class="sub_box">
            ...
            <div class="sub_box">
              <p class="sum_tit">미수금합계(%)</p>
              <p class="sub_txt" id="outstanding"></p>
            </div>                        
            <div class="sub_box">
              <p class="sum_tit">누적 미수금합계</p>
              <p class="sub_txt" id="outstandingSum"></p>
            </div>                                    
          </div>
        </div>
        <!-- 필터 영역 끝 -->
