...
@Component({
    ...
    templateUrl: './inherited-grid.component.html',
    //
    // It is preferable to declare custom 'inputs' and 'outputs' by Input and Output decorators.
    // Inherited from wijmo 'inputs' and 'outputs' should not be declared here (Ivy does not require the declaration).
    ...
    providers: [
        { provide: 'WjComponent', useExisting: forwardRef(() => InheritedGridComponent) },
        ...wjFlexGridMeta.providers
    ]
})
export class InheritedGridComponent extends WjFlexGrid {
    ...
    onFormatItem(e: wjcGrid.FormatItemEventArgs) {
        ...
        if (e.panel.cellType === wjcGrid.CellType.Cell) {
            const column: wjcGrid.Column = this.columns[e.col];
