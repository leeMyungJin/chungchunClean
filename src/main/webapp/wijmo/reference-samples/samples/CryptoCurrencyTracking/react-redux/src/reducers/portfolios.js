...
export default createReducer(initialState, {
     ...
     */
    [Action.ADD_PORTFOLIO]: (state, payload) => (Object.assign({}, state, { list: [...state.list, { name: payload, symbols: [] }] })),
    /**
     * Delete portfolio
     * @param {string] payload - The name of the portfolio
     */
    [Action.DELETE_PORTFOLIO]: (state, payload) => {
        ...
        return Object.assign({}, state, { list, selected: state.selected === payload ? list[0].name : state.selected });
    },
    /**
     * Get all the information of a portfolio
     * @param {string] payload - The name of the portfolio
     */
    [Action.SELECT_PORTFOLIO]: (state, payload) => (Object.assign({}, state, { selected: payload })),
    /**
     * Edit a portfolio
     ...
     */
    [Action.EDIT_PORTFOLIO]: (state, payload) => (Object.assign({}, state, { list: state.list.map(portfolio => (portfolio.name === payload.name ? payload.data : portfolio)) })),
});
