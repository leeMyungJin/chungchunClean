import { Globalize, PrintDocument, escapeHtml } from '@grapecity/wijmo';
import { CellType, HeadersVisibility } from '@grapecity/wijmo.grid';
...
function renderTable(flex) {
    ...
    const head = [
        '<thead>',
        flex.columnHeaders.rows.map((row, index) => renderRow(flex.columnHeaders, index)).join(''),
        '</thead>',
    ].join('');
    // body
    const body = [
