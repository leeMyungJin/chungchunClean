...
export default createReducer(initialState, {
    [Action.RECEIVE_TRADING_DATA]: (state, payload) => (Object.assign({}, state, { trading: payload })),
    [Action.RECEIVE_TICKERS_DATA]: (state, payload) => (Object.assign({}, state, { tickers: payload })),
    [Action.CHANGE_FILTER_TEXT]: (state, payload) => (Object.assign({}, state, { filter: payload })),
    [Action.CHANGE_COLUMN_VISIBILITY]: (state, payload) => {
        const { section, binding, visibility } = payload;
        const columns = Object.assign({}, state.columns, { [section]: state.columns[section].map(col => (col.binding === binding ? Object.assign({}, col, { visible: visibility }) : col)) });
