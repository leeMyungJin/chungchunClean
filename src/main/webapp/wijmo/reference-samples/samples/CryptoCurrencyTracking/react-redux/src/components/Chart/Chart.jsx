...
// wijmo components
import { ChartElement, ChartType, Position, SelectionMode } from '@grapecity/wijmo.chart';
import { FlexChart } from '@grapecity/wijmo.react.chart';
...
class Chart extends React.Component {
    constructor() {
        super(...arguments);
        this.state = { axisMin: null, axisMax: null };
        /**
         ...
         */
        this.handleItemFormatter = (engine, hitTestInfo, defaultFormat) => {
    ...
    render() {
        ...
        return (<div className="chart">
                <FlexChart className="flex-chart" chartType={ChartType.Candlestick} selectionMode={SelectionMode.Point} itemsSource={dataItem.history} series={[{ binding: 'high,low,open,close' }]} axisX={{ position: Position.None, min: axisMin, max: axisMax, reversed: true }} legend={{ position: Position.None }} itemFormatter={this.handleItemFormatter}/>
                <FlexChart className="data-range" plotMargin="6 0 6 80" itemsSource={dataItem.history} series={[{ binding: 'volume' }]} axisX={{ position: Position.None }} axisY={{ position: Position.None }} legend={{ position: Position.None }} itemFormatter={this.handleItemFormatter}>
                    <FlexChartRangeSelector rangeChanged={this.handleRangeChanged} initialized={this.handleRangeChanged}/>
                </FlexChart>
            </div>);
    }
}
