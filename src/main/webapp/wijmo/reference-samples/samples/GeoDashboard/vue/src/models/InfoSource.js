...
export class InfoSource {
  shortUrl; // url used to retrieve data/tiles
  trimName; // remove this part from item names
  values; // object that contains the values
  list; // list with range values
  legends;
  shortList;
  URL_FORMAT = 'https://services.arcgisonline.com/ArcGIS/rest/services/Demographics/SHORT_URL/MapServer';

  static getTapestryDescription(index) {
  ...
  static convertColor(array) {
    if (Array.isArray(array) && array.length >= 3) {
      return (
        '#' +
        (array[0].toString(16).length === 1 ? '0' + array[0].toString(16) : array[0].toString(16)) +
  ...
  constructor(shortUrl, keys, listKeys, trimName) {
    ...
    let arr = listKeys != null ? listKeys.split(',') : null;
    const startKey = arr != null ? arr[0] : null;
    const endKey = arr != null ? arr[1] : null;
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      const val = new InfoValue(key, null);
      this.values[key] = val;
    ...
    wjcCore.httpRequest(url, {
      success: xhr => {
        ...
        if (data.layers) {
          ...
          const drawInfos =
            data.layers[1].drawingInfo.renderer.classBreakInfos || data.layers[1].drawingInfo.renderer.uniqueValueInfos;

          for (let i = 0; i < drawInfos.length; i++) {
            const drawInfo = drawInfos[i];
            if (!self.legendExists(drawInfo.label)) {
              const index = drawInfo.label.indexOf('(');
              let label = drawInfo.label;
              ...
              self.legends.push({
                label,
                color: InfoSource.convertColor(drawInfo.symbol.color),
              });
            }
          }
        }
  ...
  getData(currentExtent, infoScale, serviceCallback) {
    ...
    if (currentExtent) {
      return esriLoader
        ...
        .then(([QueryTask, Query]) => {
          ...
          query.geometry = currentExtent.center;
          query.outFields = [];
          ...
          // run query
          const url = this.getQueryUrl(infoScale);
          const queryTask = new QueryTask(url);
          return queryTask.execute(query);
        })
        .then(featureSet => {
          this.gotData(featureSet, serviceCallback);
        });
    }
  }
   ...
   */
  gotData(featureSet, serviceCallback) {
    if (featureSet.features.length > 0) {
      // get attributes
      const atts = featureSet.features[0].attributes;

      // update values in our data object
      for (const key of Object.keys(this.values)) {
        const infoValue = this.values[key];
      ...
      if (this.shortList != null) {
        for (let i = 0; i < this.shortList.length; i++) {
          const shortItem = this.shortList[i];
          let value = 0;
          const keys = shortItem.key.split(',');
          for (let j = 0; j < keys.length; j++) {
            const key = keys[j];
            const iv = this.values[key];
            if (iv && iv.value) {
              value += iv.value;
            }
          }
          shortItem.value = value;
        }
      }
    } else {
   ...
   */
  getLocationName() {
    return this.values.NAME.value ? this.values.NAME.value + ', ' + this.values.ST_ABBREV.value : null;
  }
