...
import { InfoSource, InfoValue } from '../models/info-source.model';
...
export class DashService {
  ...
  getData() {
    ...
    if (currentExtent) {
      ...
      for (const key of Object.keys(this.infoSources).filter(el => el !== 'populationData' && el !== 'householdData')) {
        this.infoSources[key].getData(
          currentExtent,
          this.infoScale,
  ...
  gotData() {
    ...
    this.infoSources.householdIncome.shortList.forEach(
      (item, idx) => { householdData[idx] = item.value; }, [{}]
    );

    this.infoSources.householdData = [householdData];

    this.infoSources.populationData = [
      {
        label: 'Female',
        value: `${wjcCore.Globalize.format(
          this.infoSources.populationBySex.values.PFEMALE_CY.value,
          'n0'
        )}%`,
      },
      {
        label: 'Male',
        value: `${wjcCore.Globalize.format(
          this.infoSources.populationBySex.values.PMALE_CY.value,
          'n0'
        )}%`,
      },
    ];
  ...
  initService(gotDataCallback: (...args: any[]) => void): Promise<void> {
    return esriLoader.loadModules(['esri/geometry/Extent']).then(([Extent]) => {
      ...
      this.infoSources = {
        ...
        populationBySex: new InfoSource(
          'USA_Population_by_Sex',
          'TOTPOP_CY,PMALE_CY,PFEMALE_CY,MALES_CY,FEMALES_CY'
        ),
        age: new InfoSource(
          'USA_Median_Age',
          'TOTPOP_CY,MEDAGE_CY,POP0_9,POP10_19,POP20_29,POP30_39,POP40_49,POP50_59,POP60_69,POP70_79,POP80_plus',
          'POP0_9,POP80_plus',
          '2012 Total Population Age'
        ),
        householdIncome: new InfoSource(
          'USA_Median_Household_Income',
          'TOTPOP_CY,MEDHINC_CY,HINCBASECY,HINC50_CY,HINC75_CY,HINC100_CY,HINC150_CY,HINC200_CY,MEDHHINC_pct_USAvg,HINC0_25,HINC25_50',
          'HINC50_CY,HINC200_CY',
          '2012 Household Income'
        ),
        netWorth: new InfoSource(
          'USA_Median_Net_Worth',
          'TOTPOP_CY,MEDVAL_I,MEDHINC_I,MEDNW_CY,MEDNW_I,NWBASE_CY,NW0_CY,NW15_CY,NW35_CY,NW50_CY,NW75_CY,NW100_CY,NW150_CY,NW250_CY,NW500_CY,MEDNETWORTH_pct_USAvg',
          'NW0_CY,NW500_CY',
          '2012 Net Worth'
        ),
        homeValue: new InfoSource(
          'USA_Median_Home_Value',
          'TOTPOP_CY,MEDVAL_CY,MEDVAL_I,MEDHINC_I,MEDNW_I,VALBASE_CY,VAL0_CY,VAL50K_CY,VAL100K_CY,VAL150K_CY,VAL200K_CY,VAL250K_CY,VAL300K_CY,VAL400K_CY,VAL1M_CY,MEDHMVAL_pct_USAvg,VAL500_1M',
          'VAL0_CY,VAL1M_CY',
          '2012 Home Value'
        ),
      };
  ...
  setExtent(extent) {
    if (extent && this.currentExtent !== extent) {
      const sameExtent =
        extent.xmin === this.currentExtent.xmin &&
        extent.ymin === this.currentExtent.ymin &&
        extent.xmax === this.currentExtent.xmax &&
        extent.ymax === this.currentExtent.ymax;
      if (!sameExtent) {
      ...
      .then(([webMercatorUtils]) => {
        ...
        return {
          ...
          name: this.locationName
            ? this.locationName
            : 'Please select a location within the USA.',
