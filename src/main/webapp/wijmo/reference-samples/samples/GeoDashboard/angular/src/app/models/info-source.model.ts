...
export class InfoSource {
    ...
    values: any;           // object that contains the values
    list: any[];             // list with range values
    legends: any[];
    shortList: any[];
    URL_FORMAT = 'https://services.arcgisonline.com/ArcGIS/rest/services/Demographics/SHORT_URL/MapServer';

    static getTapestryDescription(index: number) {
    ...
    constructor(shortUrl, keys, listKeys?: any, trimName?: string) {
        ...
        wjcCore.httpRequest(url, {
            success: (xhr) => {
                ...
                if (data.layers) {
                    ...
                    const drawInfos = data.layers[1].drawingInfo.renderer.classBreakInfos ||
    ...
    private getData(currentExtent: any, infoScale: string, serviceCallback: () => void): Promise<void> {
     ...
     */
    private gotData(featureSet: any, serviceCallback: () => void) {
        if (featureSet.features.length > 0) {
            ...
            if (this.shortList != null) {
                for (const shortItem of this.shortList) {
                    let value = 0;
