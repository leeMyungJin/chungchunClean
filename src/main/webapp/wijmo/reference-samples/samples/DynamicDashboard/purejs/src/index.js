...
window.onload = function () {
    ...
    // color palette
    var palette = ['#8e99f3', '#ffca28', '#5c6bc0', '#bbdefb'];
    // initialize widgets
    var widgets = [
        {
            name: 'Grid',
    ...
    function addMenu(item) {
        ...
        var htmlMenuItem = core.httpRequest('./partials/menuItem.html', { async: false }).responseText;
        // initialize menu title and content
        htmlMenuItem = htmlMenuItem.replace(new RegExp('{{title}}', 'g'), item.name);
        htmlMenuItem = htmlMenuItem.replace(new RegExp('{{icon}}', 'g'), item.icon);
        // identify color palette
        palette.forEach((color, index) => {
            htmlMenuItem = htmlMenuItem.replace(new RegExp(`{{palette-${index}}}`, 'g'), color);
        });
        var menuItem = core.createElement(htmlMenuItem);
        menuItem.addEventListener('click', () => addTile(item.name));
        menu.appendChild(menuItem);
    }
    // add a tile of a given type to the dashboard
    function addTile(widgetName) {
        // get the html fragment for a new tile
        var template = widgetName.replace(' ', '');
        var htmlTile = core.httpRequest('./partials/tile.html', { async: false }).responseText;
        var htmlContent = core.httpRequest(`./partials/${template}.html`, { async: false }).responseText;
