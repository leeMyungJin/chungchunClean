...
let _products = ['Widget', 'Gadget', 'Doohickey'];
let _colors = ['Black', 'White', 'Red', 'Green', 'Blue'];
let _musicians = 'Paul,Mark,Pete,Ringo,Luke,Jacob,John,Nate,Zym,George,Toom,Crash,Boom,Dewd'.split(',');
let _someCountries = ['US', 'Germany', 'UK', 'Japan', 'Italy', 'Greece'];
let _allCountries = [
...
let _cities = 'York,Paris,Rome,Cairo,Florence,Sidney,Hamburg,Vancouver'.split(',');
// data used to generate random items
export function getData(count, allCountries = false) {
    var countries = allCountries ? _allCountries : _someCountries, data = [];
...
export function getSomeCountries() {
...
export function getMultiRowData() {
    var orders = [], today = new Date(), customers = [], firstNames = 'Aaron,Paul,John,Mark,Sue,Tom,Bill,Joe,Tony,Brad,Frank,Chris,Pat'.split(','), lastNames = 'Smith,Johnson,Richards,Bannon,Wong,Peters,White,Brown,Adams,Jennings'.split(','), states = 'SP,RS,RN,SC,CS,RT,BC'.split(','), shippers = [
    ...
    for (var i = 0; i < 50; i++) {
        var first = firstNames[randBetween(0, firstNames.length - 1)], last = lastNames[randBetween(0, lastNames.length - 1)];
        customers.push({
            id: i,
            name: first + ' ' + last,
            address: randBetween(100, 10000) + ' ' + lastNames[randBetween(0, lastNames.length - 1)] + ' St.',
            city: _cities[randBetween(0, _cities.length - 1)],
    ...
    for (var i = 0; i < 20; i++) {
        var shipped = wjcCore.DateTime.addDays(today, -randBetween(1, 3000));
        orders.push({
            id: i,
            date: wjcCore.DateTime.addDays(shipped, -randBetween(1, 5)),
            shippedDate: shipped,
...
export function getFinanciaChartData() {
    return [
...
export function getFinanciaChartType() {
...
export function getGroupCVData() {
    var data = [{
            type: 'Music',
            items: [{
                    type: 'Country',
