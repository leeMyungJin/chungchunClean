...
function init() {
    ...
    var view = new wjCore.CollectionView(data, {
        ...
        groupDescriptions: [
            new wjCore.PropertyGroupDescription('last', function (item, propName) {
        ...
        formatItem: function (s, e) {
            if (e.panel == s.cells && s.columns[e.col].binding == 'last') {
                ...
                e.cell.innerHTML = row instanceof wjGrid.GroupRow
                    ? wjCore.format('&nbsp;&nbsp;<b>{name}</b>', item)
                    : wjCore.format('{first} <b>{last}</b>', item);
            }
        }
    });
    //
                ...
                if (row instanceof wjGrid.GroupRow && row.dataItem.name == initial) {
                    ...
                    if (true) {
                        wjCore.animate(function (pct) {
                            ...
                            theGrid.scrollPosition = new wjCore.Point(pt.x, y);
                        }, 300);
                    }
                    else { // or without
                        theGrid.scrollPosition = new wjCore.Point(pt.x, -rc.top);
                    }
                    break;
                }
            }
