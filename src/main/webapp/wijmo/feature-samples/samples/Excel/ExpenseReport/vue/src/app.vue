<template>
    <div class="container-fluid">
        <div class="row">
            <button v-on:click="saveExpenses()" class="btn btn-default">Save Report</button>
        </div>
    </div>
</template>
...
<script>
    ...
    let App = Vue.extend({
        ...
        data: function() {
            return {
                employees: getEmployeesWithExpences()
            };
        },
        methods: {
            saveExpenses() {
                let workbook = this._exportExpenseReport(this.employees);
