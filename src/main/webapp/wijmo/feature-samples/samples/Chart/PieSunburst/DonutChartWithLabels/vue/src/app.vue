<template>
    <div class="container-fluid">
        <wj-flex-pie header="Best-selling Mobile Phones Brands of 2017" bindingName="brand" binding="sales"
            :innerRadius=0.5 :itemsSource="data"
            ...
            <wj-flex-pie-data-label position="Outside" :content="getLabelContent"></wj-flex-pie-data-label>
        </wj-flex-pie>
    </div>
</template>
...
<script>
    ...
    new Vue({
        ...
        methods: {
            getLabelContent(ht) {
                return wijmo.format('{name} {val:p1}', { name: ht.name, val: ht.value / this.sum });
            }
        },
        created() {
