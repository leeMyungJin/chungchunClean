...
import { FlexChart, FlexChartCore, IRenderEngine, HitTestInfo, Palettes, RenderEventArgs } from '@grapecity/wijmo.chart';
//
@Component({
...
export class AppComponent {
    ...
    ganttItemFormatter = (engine: IRenderEngine, ht: HitTestInfo, defaultFormatter: Function) => {
        ...
        if (isNumber(task.percent) && task.percent > 0) {
            let pct = clamp(task.percent, 0, 100) / 100,
    ...
    //
    private _drawConnectingLine(engine: IRenderEngine, chart: FlexChart, task: TTask, parent: TTask) {
    ...
    //
    private _getTaskRect(chart: FlexChartCore, task: TTask) {
