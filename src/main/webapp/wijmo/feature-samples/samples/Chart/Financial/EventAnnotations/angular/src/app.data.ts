...
export class DataService {
    getData() {
        ...
        for (var i = 0; i < data.length; i++) {
            data[i].date = <any>core.Globalize.parseDate(data[i].date, 'MM/dd/yy');
        }
        return data;
    }
    //
    getAnnotations() {
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i].start && data[i].start.x) {
                data[i].start.x = <any>core.Globalize.parseDate(data[i].start.x, 'yyyy-MM-dd');
            }
            if (data[i].end && data[i].end.x) {
                data[i].end.x = <any>core.Globalize.parseDate(data[i].end.x, 'yyyy-MM-dd');
            }
            if (data[i].attachment === 1 && data[i].point && data[i].point.x) {
                data[i].point.x = <any>core.Globalize.parseDate(<any>data[i].point.x, 'yyyy-MM-dd');
            }
        }
        return data;
    }
