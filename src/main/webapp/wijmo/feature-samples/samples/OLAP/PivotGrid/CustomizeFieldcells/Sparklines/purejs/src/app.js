...
function init() {
    ...
    function formatItem(s, e) {
        ...
        if (e.panel == s.cells && ng.valueFields.length) {
            ...
            let field = ng.valueFields[e.col % ng.valueFields.length], item = s.rows[e.row].dataItem, binding = s.columns[e.col].binding, spark = field.header.match(/Sparklines|Sparkbars/) != null;
