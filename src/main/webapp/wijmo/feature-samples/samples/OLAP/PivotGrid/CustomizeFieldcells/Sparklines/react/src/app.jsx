...
class App extends React.Component {
    ...
    formatItem(s, e) {
        // we want value fields in the cells panel
        let g = this._grid, ng = g.engine;
        if (e.panel == g.cells && ng.valueFields.length) {
            // we want the 'Sparklines' and 'Sparkbars' value fields
            let field = ng.valueFields[e.col % ng.valueFields.length], item = g.rows[e.row].dataItem, binding = g.columns[e.col].binding, spark = field.header.match(/Sparklines|Sparkbars/) != null;
            ...
            if (spark) {
                ...
                // we don't have the data yet, so go get it
                if (!item.sparkData) {
                    e.cell.innerHTML = '';
                    setTimeout(() => {
                        let detail = ng.getDetail(item, binding), len = detail.length;
